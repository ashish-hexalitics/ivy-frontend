...
const CheckInOverviewView = ({
  ...
  useEffect(() => {
    if (checkInOverview?.length > 0) {
      setComments(
        checkInOverview.filter(
          (it) =>
            it.entity_type === "check_in_overview" ||
            it.entity_type === "check_out_overview" ||
            it.entity_type === "inspection_overview"
        )[0]?.metadata?.comment
      );
      setQuestions(
        checkInOverview.filter(
  ...
  const addAnswer = (e) => {
    ...
    let default_ans = checkInOverviewQuestions?.filter(
      (ques) => ques.question === name.split("_")[0]
    )[0]?.answer;
    ...
    addCheckInOverviewResponse({
      ...
      entity_type:
        reportType?.toLowerCase().replace(/\s/g, "") === "inspectionreport"
          ? "inspection_overview"
          : reportType?.toLowerCase().replace(/\s/g, "") === "checkoutreport"
          ? "check_out_overview"
          : "check_in_overview",
      metadata: {
        comment: comments,
