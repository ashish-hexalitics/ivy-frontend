...
const UtilitiesView = ({ handleUtilitiesClose, utilityTypes }) => {
  ...
  const { utilities, addUtility, updateUtility, getDocLink, deleteItem, getUtilities, getCurrentReportStatus } =
    useReportState();
  ...
  useEffect(() => {
    setConfirmationCheckbox(item?.skip_utilities);
  }, [item]);
      ...
      <div className="flex flex-col gap-6 w-full md:w-[calc(100%_-_245px)] bg-[#fff] h-screen overflow-y-scroll md:px-4 pt-4 md:pt-10">
        ...
        >
          ...
          <div className="flex justify-center md:justify-between w-full items-center mr-0 md:mr-8 mb-[200px] md:mb-10 wrap px-4">
            <div
              className={`${utilities?.length > 0 && "pointer-events-none"} ${utilities?.length > 0
                ? "text-[#d3d3d3]"
            ...
            >
              ...
              <Checkbox
                ...
                label={"Confirm you want to skip utilities in this report"}
                value={utilities?.length === 0 && confirmationCheckbox}
              />
            </div>
